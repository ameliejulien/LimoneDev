...
function initFiltres(map, markers, adresses) {
    ...
    document.querySelectorAll('.vendeur').forEach(vendeur => {
        vendeur.addEventListener('change', (e) => {
            filtrerProduits();
        });
    });
...
if (typeof L !== 'undefined') {
    ...
    async function recupAdresses() {
        ...
        if (reponseAdresses.ok) {
            return await reponseAdresses.json();
        } else {
            return null;
        }
    }
    ...
    // Vendeur : pas de carte, on initialise quand même les filtres
    initFiltres(null, null, null);
}